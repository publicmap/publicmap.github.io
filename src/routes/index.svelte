<script>
  ...
  let settings = {
    ...
    map: {
		accessToken: "pk.eyJ1IjoicGxhbmVtYWQiLCJhIjoiY2todmxnbDFyMDFmcDJ5cGthZ2Vqenh3OSJ9.Plw3yyu23yPZFPIiuatpKQ",
      	worldview: "US", // Set worldview to use for disputed areas
     	style: "mapbox://styles/planemad/ckgopajx83l581bo6qr5l86yg",
		locationContext: null,
		filter: {
			iso_3166_1: null,
			iso_3166_1_name: null,
			iso_3166_2: null,
			iso_3166_2_name: null,
		}
    },
  };
  ...
  function getLocationContext(e) {
	...
	let querylngLat = map.getMap().getCenter()
	let reverseGeocodingUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${querylngLat.lng}%2C${querylngLat.lat}.json?access_token=${settings.map.accessToken}&cachebuster=1606707965137&autocomplete=true&worldview=${settings.map.worldview.toLocaleLowerCase()}&types=country%2Cregion%2Cdistrict%2Cpostcode%2Clocality%2Cplace%2Cneighborhood%2Caddress%2Cpoi&language=${settings.user.language}`
  ...
  <section class="uk-padding-small">
	...
	bind:this={geocoder}
      ...
      options={{
		...
		countries: settings.map.filter.iso_3166_1
	  }}
